...
- [ConfigurableEntityCard.tsx:54-381](file://src/app/components/dashboard/cards/shared/ConfigurableEntityCard.tsx#L54-L381)
...
- [tailwind.css:1-21](file://src/styles/tailwind.css#L1-L21)
...
- [ConfigurableEntityCard.tsx:1-381](file://src/app/components/dashboard/cards/shared/ConfigurableEntityCard.tsx#L1-L381)
...
- 可配置实体卡片：通用卡片容器，负责实体网格渲染、状态格式化、刷新与设置面板交互。**新增** 智能视觉状态检测、脉冲动画系统、动态图标选择和优化布局设计。
...
- 自定义图标：支持多种图标源（SVG、MDI、Lucide），并根据状态提供不同的视觉表现。
...
- 卡片层负责布局与配置持久化
- 实体层负责状态格式化与渲染，**新增** 智能视觉状态检测和动态图标选择
- 时间戳层负责新鲜度与离线判定
...
- **智能视觉状态检测**：新增`getEntityVisualStatus`函数，根据实体类型和状态智能判断视觉状态
  - 触发状态：二进制传感器处于触发状态
  - 正常状态：二进制传感器处于正常状态
  - 活跃状态：普通传感器处于开启状态
  - 默认状态：其他情况
- **脉冲动画系统**：当实体处于触发状态时，卡片容器应用`animate-sensor-pulse`动画
- **动态图标选择**：新增`getStatusIcon`函数，根据device_class和触发状态动态选择合适的图标
  - 门/窗/开口：触发时显示打开图标，正常时显示关闭图标
  - 人体/占用/存在：触发时显示检测到人员图标，正常时显示普通用户图标
  - 烟雾/燃气/一氧化碳：触发时显示防护盾牌图标，正常时显示默认图标
  - 湿度：触发时显示水滴图标，正常时显示默认图标
- **优化布局设计**：改进了实体网格的布局和响应式设计，支持动态高度调整
...
+getEntityVisualStatus(entityId, state) EntityVisualStatus
+getStatusIcon(deviceClass, isTriggered, fallbackIcon) string
...
ConfigurableEntityCard --> CustomIcon : "渲染动态图标"
...
- [ConfigurableEntityCard.tsx:39-52](file://src/app/components/dashboard/cards/shared/ConfigurableEntityCard.tsx#L39-L52)
- [ConfigurableEntityCard.tsx:58-64](file://src/app/components/dashboard/cards/shared/ConfigurableEntityCard.tsx#L58-L64)
- [ConfigurableEntityCard.tsx:90-112](file://src/app/components/dashboard/cards/shared/ConfigurableEntityCard.tsx#L90-L112)
- [ConfigurableEntityCard.tsx:315-327](file://src/app/components/dashboard/cards/shared/ConfigurableEntityCard.tsx#L315-L327)
...
### 自定义图标组件（CustomIcon）
- 支持多种图标源：SVG、MDI、Lucide Icons
- **状态驱动的颜色**：根据状态提供不同的颜色方案
  - default：默认灰色
  - active：主色调（蓝色）
  - alarm：警告红色
- 透明度和过渡效果：支持平滑的颜色和透明度过渡

**章节来源**
- [CustomIcon.tsx:1-74](file://src/app/components/dashboard/cards/shared/CustomIcon.tsx#L1-L74)

### 动画系统（tailwind.css）
- **传感器脉冲动画**：新增`animate-sensor-pulse`类，实现呼吸灯效果
- 动画定义：2秒周期的透明度变化，模拟呼吸灯效果
- 应用场景：当传感器处于触发状态时自动启用
...
- 组件耦合：
  ...
  - ConfigurableEntityCard 依赖 SensorTimestamp、CardSettingsPanel、CustomIcon
...
- **新增** 动画性能：
  - 脉冲动画使用CSS动画而非JavaScript，性能更优
  - 动态图标选择在渲染时计算，避免运行时状态管理
...
- 动画不生效
  ...
  - **新增** 检查`animate-sensor-pulse`类是否正确应用到触发状态的实体卡片
- **新增** 视觉状态异常
  - 检查`getEntityVisualStatus`函数的逻辑是否正确
  - 确认状态映射表（STATUS_CONTAINER_STYLES、STATUS_VALUE_STYLES、STATUS_ICON_STATE）是否完整
- **新增** 动态图标不正确
  - 检查`getStatusIcon`函数的device_class匹配逻辑
  - 确认CustomIcon组件的状态颜色映射是否正确
...
## 结论
该传感器监控系统通过卡片化聚合、时间戳新鲜度判定、设备状态同步与云端对齐，实现了对温度、湿度、光照、空气质量与安全传感器的统一可视化与实时反馈。**本版本的重大改进包括智能视觉状态检测、脉冲动画系统、动态图标选择和优化布局设计**，显著提升了用户体验和视觉反馈效果。系统具备良好的扩展性与可配置性，适合在 Home Assistant 生态下部署与维护。
...
### 动态图标选择规则
- **门/窗/开口**：触发时显示 DoorOpen，正常时显示 DoorClosed
- **人体/占用/存在**：触发时显示 UserCheck，正常时显示 UserRound
- **烟雾/燃气/一氧化碳**：触发时显示 ShieldAlert，正常时显示默认图标
- **湿度**：触发时显示 Droplets，正常时显示默认图标
- **其他**：使用默认图标
...
### 动画系统配置
- **传感器脉冲动画**：触发状态的实体卡片自动应用`animate-sensor-pulse`类
- **动画时长**：2秒周期
- **动画效果**：透明度在1和0.6之间循环变化
- **应用场景**：二进制传感器触发状态、安全传感器激活状态

**章节来源**
- [tailwind.css:15-20](file://src/styles/tailwind.css#L15-L20)
- [ConfigurableEntityCard.tsx:328-329](file://src/app/components/dashboard/cards/shared/ConfigurableEntityCard.tsx#L328-L329)