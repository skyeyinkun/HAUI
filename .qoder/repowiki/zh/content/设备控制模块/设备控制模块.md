...
UI->>Cards : 根据设备类型渲染
Cards->>Store : 触发更新(切换/滑动/点击)
Store->>Sync : 触发同步(乐观更新/重试)
Sync->>HA : 发送服务调用(更新实体状态)
HA-->>Sync : 返回最新状态
Sync->>Store : 更新设备状态
Store-->>UI : 重新渲染
```
...
dataStore["dataStore.ts"] --> device_sync["device-sync.ts"]
...
## 结论
设备控制模块通过清晰的类型定义、统一的卡片架构与完善的工具链，实现了灯光、窗帘、空调与传感器等多类设备的高效控制与状态同步。其乐观 UI、重试与容差同步策略显著提升了交互体验与鲁棒性；自动发现与批量配置简化了设备接入流程。未来可在设备类型扩展、跨域联动与能耗统计等方面进一步增强。
