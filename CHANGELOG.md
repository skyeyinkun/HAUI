...
### 优化
- **编辑模式交互优化**:
  - **统一删除按钮位置**: 将仪表盘大卡片的删除按钮 (X) 从左上角移至右上角，与底部设备卡片保持一致。
  - **平滑抖动动画**: 减小了编辑状态下的抖动幅度（由 1.5度 降至 0.5度），使 iOS 风格的交互反馈更加自然、高端。

## [3.0.19] - 2026-03-07

### 修复与优化
- **仪表盘高度逻辑修复**:
  - **修复 Tailwind 动态类名失效**: 解决了在 `widgetRegistry.tsx` 中使用字符串插值导致 JIT 编译器无法识别 `row-span-*` 类名的问题，改为显式映射以确保高度属性生效。
  - **高度精细化调整**: 将核心卡片默认高度从 600px (`h: 6`) 优化为 300px (`h: 3`)。实测表明此高度可完美、无滚动地容纳 6 个实体参数，且视觉比例更符合 iOS 设计美学。
  - **布局对齐确认**: 经浏览器像素级审查，室内环境、家庭状态、实时日志三者在 PC/移动端均保持严格对齐。

## [3.0.18] - 2026-03-07

### 修复与优化
- **仪表盘组件尺寸一致性优化**:
  - **统一核心组件高度**: 将“家庭状态（设备状态）”和“实时日志”卡片的默认高度增加至 6 个单元，与“室内环境”保持高度对齐，提升视觉统一性。
  - **移除固定高度限制**: 删除了 `LogsWidget.tsx` 中的 `max-h-[400px]` 和 `ConfigurableEntityCard.tsx` 中的 `h-[288px]` 限制，解决卡片无法随网格拉伸的问题。
  - **元数据与初始化同步**: 更新了 `widgetRegistry.tsx` 和 `useDashboardLayout.ts` 的默认配置，确保新添加的组件遵循最新的尺寸标准。
