...
#### 主题系统增强
- **CSS 变量重构**：优化 `theme.css` 主题变量定义，增强暗色/亮色模式切换
- **Tailwind 配置扩展**：新增自定义动画、过渡效果和响应式工具类

#### 组件样式统一
- **UI 基础组件**：优化 button/card/dialog/input 组件的样式一致性
- **仪表盘组件**：统一 Header/StatCard/SensorTimestamp 等组件的视觉效果
- **Widget 组件**：优化 EnergyWidget/LogsWidget/WeatherWidget 的布局和交互反馈

#### 细节改进
- **阴影系统**：优化组件阴影层级，增强视觉层次感
- **圆角统一**：统一各组件的圆角半径规范
- **过渡动画**：优化组件状态切换的过渡动画流畅度
...
#### 设备卡片快速编辑
- **长按快速编辑菜单**：长按设备卡片 1.5 秒弹出快速编辑菜单，支持重命名、移动房间、删除设备
- **触觉反馈**：长按触发时提供震动反馈
- **智能定位**：菜单位置自动适应屏幕边界
- **全覆盖支持**：所有设备卡片类型（遥控器/灯光/窗帘/空调/传感器/普通设备）均支持长按编辑

### 🐛 问题修复

#### iOS Safari 兼容性修复
- **语音识别兼容性检测**：新增 iOS Safari 语音识别支持检测，提示具体的版本/HTTPS 要求
- **版本检测**：iOS 14.5+ 版本要求提示
- **HTTPS 检测**：iOS Safari 需要 HTTPS 安全连接的提示
- **详细错误信息**：替换通用的"浏览器不支持"为具体原因说明

#### 代码审查修复
- **useLongPress 回调签名修复**：修复 `DeviceCard.tsx` 中 `useLongPress` 回调签名不匹配导致运行时异常的问题
- **onLongPress 传递修复**：向所有设备卡片子组件传递 `onLongPress` 属性
- **重复代码清理**：移除 `App.tsx` 中未使用的重复快速编辑函数
