...
export interface UseAiChatOptions {
    ...
    /** 错误状态回调 */
    onError?: () => void;
    /** 仅保留给未来高风险人工确认流程；当前明确、白名单内的控制直接执行 */
...
3. 允许直接控制灯光、开关、窗帘、风扇、媒体播放器、空调、加湿器、扫地机器人、场景和脚本等白名单内操作，不要再要求用户二次确认。
4. 不要直接执行门锁、安防解除、重启、全屋通配符或目标不明确的操作；遇到这类请求要先说明原因并询问用户明确目标。
