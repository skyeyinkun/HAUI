# 状态管理

<cite>
**本文引用的文件**
- [src/store/dataStore.ts](file://src/store/dataStore.ts)
- [src/store/deviceStore.ts](file://src/store/deviceStore.ts)
- [src/store/uiStore.ts](file://src/store/uiStore.ts)
- [src/hooks/useDashboardManager.ts](file://src/hooks/useDashboardManager.ts)
- [src/utils/sync.ts](file://src/utils/sync.ts)
- [src/utils/device-sync.ts](file://src/utils/device-sync.ts)
- [src/hooks/useHomeAssistant.ts](file://src/hooks/useHomeAssistant.ts)
- [src/app/App.tsx](file://src/app/App.tsx)
- [src/utils/cache-manager.ts](file://src/utils/cache-manager.ts)
- [src/types/device.ts](file://src/types/device.ts)
- [src/types/room.ts](file://src/types/room.ts)
- [src/types/dashboard.ts](file://src/types/dashboard.ts)
- [src/app/components/dashboard/DeviceCard.tsx](file://src/app/components/dashboard/DeviceCard.tsx)
</cite>

## 更新摘要
**所做更改**
- 新增独立设备状态管理模块（deviceStore）章节，详细介绍设备状态的分离管理
- 新增集中式仪表盘管理功能（useDashboardManager）章节，说明其替代原有分散状态管理模式的优势
- 更新架构总览图，反映新的状态管理结构
- 增强性能优化和可维护性分析
- 更新依赖关系图，体现新的模块化架构

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖关系分析](#依赖关系分析)
7. [性能考量](#性能考量)
8. [故障排除指南](#故障排除指南)
9. [结论](#结论)
10. [附录](#附录)

## 简介
本文件系统性梳理 HAUI 的状态管理体系，围绕 Zustand 状态库的应用展开，重点覆盖三类 Store 的设计与职责：数据状态 Store（dataStore）、独立设备状态 Store（deviceStore）与 UI 状态 Store（uiStore）。文档将深入解释状态组织结构、数据模型定义、状态更新机制、跨设备同步的一致性保障与冲突解决策略、异步状态更新与副作用处理、持久化与缓存管理、内存优化以及调试与排障方法。

**更新** 新增独立设备状态管理和集中式仪表盘管理功能，提供更好的性能和可维护性。

## 项目结构
HAUI 的状态管理采用分层模块化组织：
- 数据状态 Store：负责设备、房间、场景、用户、日志等实体数据的集中管理，并通过持久化中间件实现本地存储与服务端同步。
- 独立设备状态 Store：专门管理设备状态，避免其他状态变化导致不必要的重新渲染，提供选择器优化组件订阅。
- UI 状态 Store：负责界面交互状态（如模态框开关、编辑模式、选中设备等），不涉及持久化。
- 集中式仪表盘管理：封装仪表盘的核心业务逻辑，包括设备状态管理、房间/场景切换、设备操作等，替代原有的分散状态管理模式。
- 同步工具：封装了与服务端存储的同步逻辑，包括主动推送、被动拉取、心跳与聚焦对齐。
- 设备状态同步：将 Home Assistant 实体状态映射回本地设备模型，确保视图与实际设备状态一致。
- 类型定义：统一的数据模型接口，明确字段语义与可选扩展，支撑强类型开发与运行时校验。

```mermaid
graph TB
subgraph "状态层"
DS["dataStore<br/>数据状态 Store"]
DVS["deviceStore<br/>独立设备状态 Store"]
US["uiStore<br/>UI 状态 Store"]
DD["useDashboardManager<br/>集中式仪表盘管理"]
end
subgraph "同步层"
SYNC["sync.ts<br/>服务端同步"]
DEVSYNC["device-sync.ts<br/>实体同步"]
end
subgraph "类型与工具"
TYPES["types/*<br/>数据模型"]
CACHE["cache-manager.ts<br/>缓存管理"]
END
subgraph "应用层"
APP["App.tsx<br/>应用入口"]
CARD["DeviceCard.tsx<br/>设备卡片"]
END
APP --> DS
APP --> US
APP --> DVS
APP --> DD
DS --> SYNC
DVS --> SYNC
APP --> DEVSYNC
DEVSYNC --> DS
DS --> TYPES
DVS --> TYPES
US --> TYPES
CACHE -.-> APP
```

**图表来源**
- [src/store/dataStore.ts:58-128](file://src/store/dataStore.ts#L58-L128)
- [src/store/deviceStore.ts:50-78](file://src/store/deviceStore.ts#L50-L78)
- [src/store/uiStore.ts:31-55](file://src/store/uiStore.ts#L31-L55)
- [src/hooks/useDashboardManager.ts:25-320](file://src/hooks/useDashboardManager.ts#L25-L320)
- [src/utils/sync.ts:49-161](file://src/utils/sync.ts#L49-L161)
- [src/utils/device-sync.ts:4-191](file://src/utils/device-sync.ts#L4-L191)
- [src/app/App.tsx:116-123](file://src/app/App.tsx#L116-L123)
- [src/app/components/dashboard/DeviceCard.tsx:26-200](file://src/app/components/dashboard/DeviceCard.tsx#L26-L200)

**章节来源**
- [src/store/dataStore.ts:1-129](file://src/store/dataStore.ts#L1-L129)
- [src/store/deviceStore.ts:1-91](file://src/store/deviceStore.ts#L1-L91)
- [src/store/uiStore.ts:1-55](file://src/store/uiStore.ts#L1-L55)
- [src/hooks/useDashboardManager.ts:1-320](file://src/hooks/useDashboardManager.ts#L1-L320)
- [src/utils/sync.ts:1-161](file://src/utils/sync.ts#L1-L161)
- [src/utils/device-sync.ts:1-191](file://src/utils/device-sync.ts#L1-L191)
- [src/app/App.tsx:1-200](file://src/app/App.tsx#L1-L200)
- [src/app/components/dashboard/DeviceCard.tsx:1-200](file://src/app/components/dashboard/DeviceCard.tsx#L1-L200)

## 核心组件
- 数据状态 Store（dataStore）
  - 职责：维护设备、房间、场景、用户、日志等实体集合；提供批量更新、单项更新、日志追加与清理等动作；通过持久化中间件将关键字段写入本地存储，并在变更时触发服务端同步。
  - 关键点：默认遥控器设备注入、遗留数据迁移、部分字段持久化、日志上限截断。
- 独立设备状态 Store（deviceStore）
  - 职责：专门管理设备状态，提供设备列表、设备更新、设备切换等功能；通过选择器优化组件订阅，减少不必要的重新渲染。
  - 关键点：独立持久化、设备选择器、默认遥控器设备注入、性能优化。
- UI 状态 Store（uiStore）
  - 职责：管理界面交互状态（设置面板、气候/遥控模态框、区域选择、仪表盘编辑模式等）；提供便捷的打开/关闭与带数据的打开方法。
  - 关键点：非持久化状态，轻量级动作函数，便于组件快速读写。
- 集中式仪表盘管理（useDashboardManager）
  - 职责：封装仪表盘的核心业务逻辑，包括设备状态管理、房间/场景切换、设备操作、事件日志处理等；替代原有的分散状态管理模式。
  - 关键点：业务逻辑集中管理、性能优化、可维护性提升。

**章节来源**
- [src/store/dataStore.ts:9-28](file://src/store/dataStore.ts#L9-L28)
- [src/store/dataStore.ts:58-128](file://src/store/dataStore.ts#L58-L128)
- [src/store/deviceStore.ts:6-20](file://src/store/deviceStore.ts#L6-L20)
- [src/store/deviceStore.ts:50-78](file://src/store/deviceStore.ts#L50-L78)
- [src/store/uiStore.ts:3-29](file://src/store/uiStore.ts#L3-L29)
- [src/store/uiStore.ts:31-55](file://src/store/uiStore.ts#L31-L55)
- [src/hooks/useDashboardManager.ts:12-24](file://src/hooks/useDashboardManager.ts#L12-L24)

## 架构总览
下图展示从用户操作到状态更新、持久化与跨设备同步的端到端流程，反映新的模块化架构。

```mermaid
sequenceDiagram
participant UI as "组件/用户"
participant DVS as "deviceStore"
participant DS as "dataStore"
participant LS as "localStorage"
participant SYNC as "sync.ts"
participant SVR as "服务端存储"
UI->>DVS : 触发设备状态更新
DVS->>LS : 写入设备持久化字段
LS-->>DVS : 触发 storage 事件回调
DVS->>SYNC : 调用同步到服务端
SYNC->>SVR : POST 批量键值
SVR-->>SYNC : 返回成功
SYNC-->>DVS : 更新本地时间戳
note over DVS,SYNC : 变更后 1 秒内防抖合并发送
UI->>DS : 触发其他状态更新
DS->>LS : 写入其他持久化字段
DS->>SYNC : 调用同步到服务端
```

**图表来源**
- [src/store/deviceStore.ts:50-78](file://src/store/deviceStore.ts#L50-L78)
- [src/store/dataStore.ts:106-127](file://src/store/dataStore.ts#L106-L127)
- [src/utils/sync.ts:52-93](file://src/utils/sync.ts#L52-L93)

## 详细组件分析

### 独立设备状态 Store（deviceStore）
- 设计理念
  - 专门管理设备状态，避免其他状态变化导致不必要的重新渲染
  - 通过选择器优化组件订阅，只获取需要的部分状态
  - 独立持久化，减少存储体积和序列化成本
- 数据模型与字段
  - 设备：包含标识、名称、图标、开关状态、房间归属、类型、可见性、亮度/色温、温度参数、最后更新时间等。
- 状态更新机制
  - 批量更新：支持函数式更新，避免重复渲染与竞态。
  - 单项更新：针对特定设备属性进行局部更新。
  - 设备切换：提供便捷的开关状态切换功能。
- 持久化与性能优化
  - 独立持久化，仅持久化设备字段，减少存储体积。
  - 通过选择器（selectCommonDevices、selectDevicesByRoom、selectDevicesByType）优化组件订阅。
  - 默认遥控器设备注入，确保遥控功能可用。
- 选择器优化
  - 常用设备选择器：`selectCommonDevices` - 只获取常用设备
  - 房间设备选择器：`selectDevicesByRoom` - 按房间获取设备
  - 类型设备选择器：`selectDevicesByType` - 按类型获取设备

```mermaid
flowchart TD
Start(["进入设备状态更新"]) --> Choose["判断更新类型"]
Choose --> |批量更新| Batch["函数式批量更新"]
Choose --> |单项更新| Single["按 ID 定位并合并更新"]
Choose --> |设备切换| Toggle["切换 isOn 状态"]
Batch --> Persist["写入设备持久化字段"]
Single --> Persist
Toggle --> Persist
Persist --> Debounce["1 秒防抖"]
Debounce --> Sync["触发服务端同步"]
Sync --> End(["完成"])
```

**图表来源**
- [src/store/deviceStore.ts:55-70](file://src/store/deviceStore.ts#L55-L70)
- [src/store/deviceStore.ts:10-20](file://src/store/deviceStore.ts#L10-L20)

**章节来源**
- [src/store/deviceStore.ts:6-20](file://src/store/deviceStore.ts#L6-L20)
- [src/store/deviceStore.ts:31-48](file://src/store/deviceStore.ts#L31-L48)
- [src/store/deviceStore.ts:50-78](file://src/store/deviceStore.ts#L50-L78)
- [src/store/deviceStore.ts:80-91](file://src/store/deviceStore.ts#L80-L91)

### 集中式仪表盘管理（useDashboardManager）
- 功能概述
  - 封装仪表盘的核心业务逻辑，包括设备状态管理、房间/场景切换、设备操作、事件日志处理等
  - 替代原有的分散状态管理模式，提供更好的性能和可维护性
  - 使用 useCallback 缓存处理函数，避免不必要的重新渲染
- 核心功能模块
  - 设备状态管理：集成设备同步、设备操作、设备过滤
  - 房间/场景切换：管理房间选择状态和场景冷却机制
  - 用户同步：与 Home Assistant 用户实体同步状态
  - 事件日志处理：统一的日志生成和管理
- 性能优化策略
  - 使用 useCallback 缓存处理函数
  - 使用 useMemo 缓存过滤结果
  - 使用 useRef 避免闭包问题
  - 集中管理复杂的状态更新逻辑

```mermaid
flowchart TD
Start(["初始化仪表盘管理"]) --> Init["初始化状态和引用"]
Init --> Effects["设置同步 Effects"]
Effects --> DeviceSync["设备状态同步 Effect"]
Effects --> UserSync["用户状态同步 Effect"]
DeviceSync --> Filter["设备过滤逻辑"]
UserSync --> Filter
Filter --> Ops["设备操作函数"]
Ops --> Return["返回管理接口"]
Return --> End(["完成"])
```

**图表来源**
- [src/hooks/useDashboardManager.ts:25-57](file://src/hooks/useDashboardManager.ts#L25-L57)
- [src/hooks/useDashboardManager.ts:68-75](file://src/hooks/useDashboardManager.ts#L68-L75)
- [src/hooks/useDashboardManager.ts:77-141](file://src/hooks/useDashboardManager.ts#L77-L141)

**章节来源**
- [src/hooks/useDashboardManager.ts:12-24](file://src/hooks/useDashboardManager.ts#L12-L24)
- [src/hooks/useDashboardManager.ts:25-57](file://src/hooks/useDashboardManager.ts#L25-L57)
- [src/hooks/useDashboardManager.ts:68-141](file://src/hooks/useDashboardManager.ts#L68-L141)
- [src/hooks/useDashboardManager.ts:143-318](file://src/hooks/useDashboardManager.ts#L143-L318)

### 数据状态 Store（dataStore）
- 数据模型与字段
  - 设备：包含标识、名称、图标、开关状态、房间归属、类型、可见性、亮度/色温、温度参数、最后更新时间等。
  - 房间：包含标识、名称、类型、排序等。
  - 场景与日志：场景用于场景切换，日志用于记录关键事件。
- 状态更新机制
  - 批量更新：支持函数式更新，避免重复渲染与竞态。
  - 单项更新：针对特定设备属性进行局部更新。
  - 日志管理：追加新日志并限制长度，保证内存占用可控。
- 持久化与跨设备同步
  - 仅持久化关键字段，减少存储体积。
  - 在 storage 事件回调中触发服务端同步，实现本地变更的跨设备传播。
  - 服务端同步采用防抖策略，合并短时间内多次变更。
- 默认设备注入与兼容性
  - 若无遥控器设备则自动注入一条默认遥控器设备，确保遥控功能可用。
  - 支持从 localStorage 迁移旧键名，提升兼容性。

```mermaid
flowchart TD
Start(["进入数据更新"]) --> Choose["判断更新类型"]
Choose --> |批量更新| Batch["函数式批量更新"]
Choose --> |单项更新| Single["按 ID 定位并合并更新"]
Choose --> |日志| LogOp["追加并截断到上限"]
Batch --> Persist["写入持久化字段"]
Single --> Persist
LogOp --> Persist
Persist --> Debounce["1 秒防抖"]
Debounce --> Sync["触发服务端同步"]
Sync --> End(["完成"])
```

**图表来源**
- [src/store/dataStore.ts:67-102](file://src/store/dataStore.ts#L67-L102)
- [src/store/dataStore.ts:106-127](file://src/store/dataStore.ts#L106-L127)
- [src/utils/sync.ts:52-93](file://src/utils/sync.ts#L52-L93)

**章节来源**
- [src/store/dataStore.ts:9-28](file://src/store/dataStore.ts#L9-L28)
- [src/store/dataStore.ts:30-47](file://src/store/dataStore.ts#L30-L47)
- [src/store/dataStore.ts:49-56](file://src/store/dataStore.ts#L49-L56)
- [src/store/dataStore.ts:67-102](file://src/store/dataStore.ts#L67-L102)
- [src/store/dataStore.ts:106-127](file://src/store/dataStore.ts#L106-L127)
- [src/types/device.ts:1-46](file://src/types/device.ts#L1-L46)
- [src/types/room.ts:1-33](file://src/types/room.ts#L1-L33)
- [src/types/dashboard.ts:1-12](file://src/types/dashboard.ts#L1-L12)

### UI 状态 Store（uiStore）
- 管理的 UI 状态包括：设置面板开关与默认标签页、气候/遥控模态框、区域选择模态框、仪表盘编辑模式、选中的设备等。
- 提供便捷的动作函数，如打开特定模态框并附带数据，或直接切换编辑模式。
- 该 Store 不参与持久化，确保 UI 行为与业务数据解耦。

**章节来源**
- [src/store/uiStore.ts:3-29](file://src/store/uiStore.ts#L3-L29)
- [src/store/uiStore.ts:31-55](file://src/store/uiStore.ts#L31-L55)

### 跨设备同步与一致性
- 版本控制与增量对齐
  - 使用时间戳作为版本号，仅在远端版本较新或强制同步时才覆盖本地。
  - 拉取完成后触发自定义事件，通知应用刷新状态。
- 自动同步策略
  - 心跳：每 30 秒检查一次远端版本。
  - 聚焦对齐：页面获得焦点时触发一次对齐。
- 冲突解决
  - 以远端为准的单向对齐策略，避免双向冲突。
  - 本地变更通过防抖合并后统一推送，降低并发写入风险。

```mermaid
sequenceDiagram
participant APP as "应用"
participant SYNC as "sync.ts"
participant SVR as "服务端存储"
APP->>SYNC : 请求从服务端同步
SYNC->>SVR : GET 存储数据
SVR-->>SYNC : 返回数据与版本
SYNC->>SYNC : 比较本地/远端版本
alt 远端较新或强制
SYNC->>APP : 触发自定义事件刷新
else 本地已是最新
SYNC-->>APP : 无需更新
end
```

**图表来源**
- [src/utils/sync.ts:98-131](file://src/utils/sync.ts#L98-L131)
- [src/utils/sync.ts:136-150](file://src/utils/sync.ts#L136-L150)

**章节来源**
- [src/utils/sync.ts:46-161](file://src/utils/sync.ts#L46-L161)

### 设备状态同步（HA 实体到本地模型）
- 同步范围与策略
  - 针对灯光、开关、窗帘、传感器、二进制传感器、空调等多种设备类型分别处理状态与属性映射。
  - 仅在状态或属性发生实质性变化时才更新本地模型，避免不必要的重渲染。
- 时间戳与可用性
  - 同步实体的 last_updated/last_changed，确保 UI 显示最新时间信息。
  - 将 unavailable/unknown 状态映射为本地不可用标记。
- 设备映射
  - 通过设备映射表将本地设备 ID 与 Home Assistant 实体 ID 对应，保证一对一同步。

```mermaid
flowchart TD
S0["开始同步"] --> Map["根据映射获取实体"]
Map --> Exists{"实体是否存在"}
Exists -- 否 --> Skip["保持原状"]
Exists -- 是 --> Type["按设备类型分支处理"]
Type --> Light["灯光/开关：同步开关与亮度/色温"]
Type --> Curtain["窗帘：同步开关与位置"]
Type --> Sensor["传感器：同步数值与在线状态"]
Type --> Climate["空调：同步温度/模式/风速/扫风等"]
Light --> Update["计算差异并合并更新"]
Curtain --> Update
Sensor --> Update
Climate --> Update
Update --> TS["更新时间戳/可用性"]
TS --> Save["返回新数组或原数组"]
```

**图表来源**
- [src/utils/device-sync.ts:4-191](file://src/utils/device-sync.ts#L4-L191)

**章节来源**
- [src/utils/device-sync.ts:1-191](file://src/utils/device-sync.ts#L1-L191)

### 异步状态更新与副作用处理
- WebSocket 与 REST 回退
  - 优先使用 WebSocket 获取实体与事件，失败时回退到 REST 接口。
  - 心跳检测与延迟评估，确保连接健康。
- 应用入口的自动扫描与同步
  - 连接建立后自动扫描设备并发现新增实体，随后初始化服务端同步的心跳与聚焦对齐。
- 日志与错误处理
  - 组件侧对异常进行降级处理（如自动扫描失败时重试一次），避免影响用户体验。

**章节来源**
- [src/hooks/useHomeAssistant.ts:250-312](file://src/hooks/useHomeAssistant.ts#L250-L312)
- [src/hooks/useHomeAssistant.ts:37-164](file://src/hooks/useHomeAssistant.ts#L37-L164)
- [src/app/App.tsx:289-325](file://src/app/App.tsx#L289-L325)

### 订阅机制与组件联动
- 组件订阅 Store
  - 应用入口同时订阅数据与 UI Store，实现设备列表、房间、场景、用户、日志与界面交互状态的统一管理。
  - 设备卡片根据设备类型动态渲染不同控件，响应状态变化。
- 事件驱动刷新
  - 服务端同步完成后通过自定义事件驱动 Store 刷新，确保多设备间状态一致。

**章节来源**
- [src/app/App.tsx:86-123](file://src/app/App.tsx#L86-L123)
- [src/app/components/dashboard/DeviceCard.tsx:26-200](file://src/app/components/dashboard/DeviceCard.tsx#L26-L200)
- [src/utils/sync.ts:119-121](file://src/utils/sync.ts#L119-L121)

### 缓存管理与内存优化
- 本地缓存
  - CacheManager 提供基于 localStorage 的简单缓存，支持 TTL 过期与静默读取。
- 内存优化建议
  - 仅持久化必要字段，避免冗余数据膨胀。
  - 日志列表截断，防止无限增长。
  - 防抖合并频繁写入，降低同步频率与内存抖动。
  - 使用选择器优化组件订阅，减少不必要的重新渲染。

**章节来源**
- [src/utils/cache-manager.ts:1-56](file://src/utils/cache-manager.ts#L1-L56)
- [src/store/dataStore.ts:89-91](file://src/store/dataStore.ts#L89-L91)
- [src/store/dataStore.ts:118-125](file://src/store/dataStore.ts#L118-L125)

## 依赖关系分析
- 组件与 Store
  - App.tsx 同时消费 dataStore、uiStore 和 deviceStore，形成数据、UI 和设备状态的三通道。
  - DeviceCard 根据设备类型渲染不同子组件，间接依赖 deviceStore 中的设备状态。
  - useDashboardManager 集中管理仪表盘业务逻辑，替代原有的分散状态管理模式。
- Store 与工具
  - dataStore 依赖 sync.ts 实现持久化后的服务端同步。
  - deviceStore 依赖 sync.ts 实现独立的设备状态同步。
  - 设备状态同步依赖 device-sync.ts 与 Home Assistant 实体数据。
- 类型定义
  - types/* 为所有 Store 与组件提供强类型约束，确保字段语义清晰。

```mermaid
graph LR
APP["App.tsx"] --> DS["dataStore"]
APP --> US["uiStore"]
APP --> DVS["deviceStore"]
APP --> DD["useDashboardManager"]
DS --> SYNC["sync.ts"]
DVS --> SYNC
DS --> DEVSYNC["device-sync.ts"]
DVS --> DEVSYNC
DS --> TYPES["types/*"]
DVS --> TYPES
US --> TYPES
CARD["DeviceCard.tsx"] --> DVS
DD --> DS
DD --> US
```

**图表来源**
- [src/app/App.tsx:86-123](file://src/app/App.tsx#L86-L123)
- [src/store/dataStore.ts:58-128](file://src/store/dataStore.ts#L58-L128)
- [src/store/deviceStore.ts:50-78](file://src/store/deviceStore.ts#L50-L78)
- [src/store/uiStore.ts:31-55](file://src/store/uiStore.ts#L31-L55)
- [src/hooks/useDashboardManager.ts:25-41](file://src/hooks/useDashboardManager.ts#L25-L41)
- [src/utils/sync.ts:49-161](file://src/utils/sync.ts#L49-L161)
- [src/utils/device-sync.ts:4-191](file://src/utils/device-sync.ts#L4-L191)
- [src/app/components/dashboard/DeviceCard.tsx:26-200](file://src/app/components/dashboard/DeviceCard.tsx#L26-L200)

## 性能考量
- 防抖与批处理
  - 本地变更触发的同步采用 1 秒防抖，合并短时间内多次写入，降低网络与 CPU 开销。
- 选择性持久化
  - 仅持久化关键字段，减少存储体积与序列化成本。
- 渐进式更新
  - 设备状态同步仅在状态或属性变化时更新，避免无效渲染。
- 连接健康监控
  - WebSocket 心跳与延迟评估，及时发现并恢复连接问题。
- 组件订阅优化
  - deviceStore 通过选择器优化组件订阅，减少不必要的重新渲染。
  - useDashboardManager 使用 useCallback 缓存处理函数，避免不必要的重新渲染。

**更新** 新增独立设备状态管理和集中式仪表盘管理的性能优化策略。

## 故障排除指南
- 服务端同步失败
  - 现象：同步接口报错或无响应。
  - 处理：检查网络连通性与凭据；确认服务端存储接口可达；查看浏览器控制台日志。
  - 参考
    - [src/utils/sync.ts:74-92](file://src/utils/sync.ts#L74-L92)
- 本地数据未对齐
  - 现象：多设备间状态不一致。
  - 处理：手动触发一次从服务端同步；确认版本时间戳是否更新；检查页面聚焦与心跳是否生效。
  - 参考
    - [src/utils/sync.ts:98-131](file://src/utils/sync.ts#L98-L131)
    - [src/utils/sync.ts:136-150](file://src/utils/sync.ts#L136-L150)
- 设备状态不同步
  - 现象：设备开关/数值与 Home Assistant 实体不一致。
  - 处理：确认设备映射表正确；检查实体是否存在且状态非 unavailable/unknown；查看设备类型分支是否覆盖目标设备。
  - 参考
    - [src/utils/device-sync.ts:4-191](file://src/utils/device-sync.ts#L4-L191)
- 自动扫描失败
  - 现象：启动后未能自动发现设备。
  - 处理：等待一次重试；检查 HA 连接与凭据；确认 REST/WebSocket 接口可用。
  - 参考
    - [src/app/App.tsx:289-325](file://src/app/App.tsx#L289-L325)
    - [src/hooks/useHomeAssistant.ts:250-312](file://src/hooks/useHomeAssistant.ts#L250-L312)
- 设备状态管理异常
  - 现象：设备状态更新不生效或出现性能问题。
  - 处理：检查 deviceStore 的选择器使用；确认组件订阅是否正确；验证设备映射配置。
  - 参考
    - [src/store/deviceStore.ts:80-91](file://src/store/deviceStore.ts#L80-L91)
- 仪表盘管理功能异常
  - 现象：仪表盘操作响应缓慢或功能异常。
  - 处理：检查 useDashboardManager 的回调函数缓存；确认设备过滤逻辑；验证用户同步状态。
  - 参考
    - [src/hooks/useDashboardManager.ts:68-141](file://src/hooks/useDashboardManager.ts#L68-L141)
    - [src/hooks/useDashboardManager.ts:143-318](file://src/hooks/useDashboardManager.ts#L143-L318)

**更新** 新增设备状态管理和仪表盘管理功能的故障排除指南。

## 结论
HAUI 的状态管理以 Zustand 为核心，通过引入独立设备状态管理和集中式仪表盘管理功能，实现了更好的性能和可维护性。将数据、设备状态和 UI 分离管理，配合本地持久化与服务端同步，实现了跨设备的一致性与良好的用户体验。通过类型约束、选择性持久化、防抖与增量对齐、组件订阅优化等策略，系统在性能与可靠性之间取得平衡。未来可在以下方面持续优化：引入更细粒度的选择器、增强冲突检测与回滚能力、完善调试面板与可观测性工具。

**更新** 新架构显著提升了系统的性能和可维护性，建议在新功能开发中优先考虑使用 deviceStore 和 useDashboardManager。

## 附录
- 状态调试建议
  - 使用浏览器开发者工具的 Redux DevTools 或 Zustand 自带调试插件观察状态变化。
  - 在服务端同步失败时，检查请求头与响应体，定位网络或权限问题。
  - 对设备状态异常，打印设备映射与实体属性，验证同步分支逻辑。
  - 使用 deviceStore 的选择器进行精确调试，避免全局状态干扰。
- 最佳实践清单
  - 仅持久化必要字段，避免冗余。
  - 使用函数式更新与局部更新，减少重渲染。
  - 对频繁写入进行防抖合并。
  - 严格区分数据状态与 UI 状态，避免 UI 状态污染持久化。
  - 为关键流程添加日志与错误边界，便于排障。
  - 优先使用 deviceStore 管理设备状态，使用 useDashboardManager 管理仪表盘业务逻辑。
  - 合理使用选择器优化组件订阅，提升渲染性能。