...
- **版本同步**：根前端包、Add-on 包、HA 自定义集成、Service Worker 缓存版本和运行时显示版本统一更新为 `5.15.16`。
- **UI 修复**：修复夜间墙屏全屏发灰、设备卡片图标黑边、房间/常用按钮阴影、AI 入口权重过高和控制误确认问题。

### 文档

- **发布文档同步**：README、用户手册、Add-on 文档和商业交付文档同步到 `5.15.16`。
...
- **授权签名兼容**：前端授权验证兼容 Node/OpenSSL 生成的 DER 格式 ECDSA 签名，修复授权工具生成的有效授权码在浏览器端提示“授权签名验证失败”的问题。
- **授权激活顺序**：授权页优先调用 Add-on 后端验签并持久化授权，后端不可用时才回退到本地浏览器验签，降低 Ingress 环境下的激活失败率。

### 测试

- **授权回归测试**：新增真实机器码与授权码样例测试，覆盖 DER 签名授权码解析、验签和机器码不匹配拦截。
- **发布验证通过**：TypeScript、ESLint、Vitest、Add-on Node 测试和 Add-on 构建均通过。
...
| 版本 | 日期 | 主要更新 |
|------|------|----------|
| 5.15.0 | 2026-05-14 | 修复授权码前端签名验证失败、Add-on 后端优先激活、授权回归测试 |
