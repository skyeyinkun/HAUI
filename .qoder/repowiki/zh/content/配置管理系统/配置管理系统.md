...
- [src/app/components/settings/RoomManagementTab.tsx](file://src/app/components/settings/RoomManagementTab.tsx)
...
## 更新摘要
**所做更改**
- 更新版本信息：前端版本从 3.24.0 回退到 3.12.21
- 更新依赖配置：调整关键依赖版本以匹配当前配置状态
- 更新配置文件：修正 addon 配置中的版本号和依赖版本
...
**更新** 版本信息已根据当前实际配置状态进行更新，确保文档中的版本信息准确反映系统的实际状态。
...
- [device.ts:1-119](file://src/types/device.ts#L1-L119)
...
**更新** 版本信息已更新为 3.12.21，确保与当前实际配置状态一致。
...
**更新** 依赖版本已调整以匹配当前配置状态，确保系统性能与稳定性。
...
**更新** 版本信息已更新为 3.12.21，故障排查指南中的版本参考已相应调整。
...
**更新** 当前版本为 3.12.21，确保与实际配置状态保持一致，为用户提供准确的版本信息。

## 附录
- 初始设备配置示例：包含空调、灯具、窗帘、传感器与遥控器等常见设备的默认参数。
- 类型定义参考：设备、房间、用户与卡片配置的数据结构，便于扩展与集成。
- 版本信息：当前系统版本为 3.12.21，依赖配置已调整以确保兼容性与稳定性。

**更新** 附录中的版本信息已更新为 3.12.21，确保与当前实际配置状态一致。
...
- [addon/config.yaml:2](file://addon/config.yaml#L2)