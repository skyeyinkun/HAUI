...
![version](https://img.shields.io/badge/version-5.15.16-blue.svg)
...
5.15.16 是本轮 UI 修复与版本同步版本，重点修复夜间墙屏发灰、卡片图标黑边、房间按钮阴影、AI 入口降权与控制逻辑误确认问题。

- **版本统一**：根前端包、Add-on 包、HA 自定义集成、Service Worker 缓存版本和界面底部版本号统一为 `5.15.16`。
- **黑白玻璃质感**：仪表盘、导航、卡片、设置和授权界面沿用磨砂玻璃视觉风格，并收紧不必要的黑边阴影。
- **当前功能保持**：Home Assistant Add-on / Ingress 面板仍为主入口，PWA 与平板墙屏兼容，功能边界不变。

## 5.15.0 发布重点

5.15.0 是授权激活稳定性修复版本，重点解决授权工具生成的有效授权码在浏览器端提示“授权签名验证失败”的问题，并继续保持 5.14.0 的 Add-on 优先商业交付模型。

- **授权签名兼容**：前端 WebCrypto 验签兼容 Node/OpenSSL 生成的 DER 格式 ECDSA 签名，已生成的授权码无需重新生成。
- **Add-on 优先激活**：授权页优先调用 Add-on 后端验签并持久化授权，后端不可用时才回退到本地浏览器验签。
- **机器码绑定保持不变**：仍然绑定 Add-on 后端持久化机器码，换浏览器或平板访问不会改变机器码。
- **授权回归测试**：新增真实机器码与授权码样例测试，覆盖授权码解析、签名验证和机器码不匹配拦截。
- **交付路线不变**：继续推荐 Home Assistant Add-on / Ingress 面板为主入口，PWA 和平板墙屏兼容，暂不推进原生 App 商店包。
...
- 目标明确且白名单内的 AI 控制会直接提交 Home Assistant 服务，无需二次确认。
...
- 部署后先显示授权页面，授权通过后才进入 HAUI 主界面。
- 授权绑定单个 Home Assistant 实例，换实例需要重新生成授权码。
...
该命令会输出 `customer-packages/haui-addon-v当前版本`，只包含 Home Assistant Add-on 运行所需文件，不包含 `src`、`scripts`、测试、开发配置、授权生成工具和私钥。商业交付时应发送该交付包或由你远程部署，不要发送完整开发仓库。
...
HAUI 开发仓库只保留授权验证逻辑和公钥配置入口，不再包含授权码生成脚本。授权生成工具应放在开发者本机私有目录，例如项目同级的 `HAUI-License-Tools`，并和客户交付包分开管理。

客户交付时只需要：

- HAUI Add-on 运行包。
- `HAUI_LICENSE_PUBLIC_KEY` 公钥配置。
- 绑定客户机器码生成的授权码。

不要向客户发送完整开发仓库、`scripts` 目录、`src` 目录、私钥或授权生成工具。
...
- [商业交付隔离方案](./docs/COMMERCIAL_DELIVERY.md)
