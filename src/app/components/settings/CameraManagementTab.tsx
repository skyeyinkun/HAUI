...
export function CameraManagementTab({ cameras, onUpdateCameras, entitiesRegistry = [] }: CameraManagementTabProps) {
  ...
  const testCamera = async (camera: CameraConfig) => {
    ...
    try {
      if (camera.type === 'ha-hls') {
        if (!camera.url && !camera.entityId) throw new Error('请填写 HLS 地址或 HA 实体 ID');
        const target = camera.url || getApiUrl(`/ha-api/api/camera_proxy_stream/${camera.entityId}`);
        const res = await fetch(target, { method: 'GET', signal: AbortSignal.timeout(5000) });
        if (!res.ok && res.status !== 405) throw new Error(`连接失败：${res.status}`);
      } else if (camera.type === 'rtsp') {
        if (!camera.go2rtcUrl || !camera.streamName) throw new Error('请填写 go2rtc 地址和流名称');
        const base = camera.go2rtcUrl.replace(/\/$/, '');
        const res = await fetch(`${base}/api/streams`, { signal: AbortSignal.timeout(5000) });
        if (!res.ok) throw new Error(`go2rtc 不可用：${res.status}`);
      } else if (camera.type === 'ezviz') {
        const res = await fetch(getApiUrl('/api/ezviz/token'), {
          method: 'POST',
          credentials: 'include',
        ...
        if (!res.ok) throw new Error(await readApiError(res, '萤石云连接测试失败'));
      }
      toast.success(`${camera.name} 连接测试通过`);
    } catch (error) {
      ...
      <div className="space-y-6 max-w-4xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pb-2 border-b border-gray-100">
          <div>
            ...
            <p className="text-[12px] text-gray-500 mt-1">配置全屋监控，优先使用 Home Assistant 本地代理；摄像头画面默认不经过 HAUI 云端。</p>
          </div>
          <button
        ...
        <div className="space-y-4 pb-8">
          {cameras.map((camera) => (
            ...
            >
                ...
                <div className="flex flex-col md:flex-row md:items-center gap-4">
                  ...
                  <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-3 lg:gap-4">
                    <div className="space-y-1.5">
                      ...
                      <input
                        type="text"
                        value={camera.entityId || ''}
                        onChange={(e) => handleUpdateCamera(camera.id, { entityId: e.target.value })}
                  ...
                  <div className="flex items-center justify-end shrink-0 pl-2">
                    <button
                      ...
                      className="mr-1 p-2.5 text-gray-300 hover:text-[#334155] hover:bg-gray-50 rounded-xl transition-all opacity-40 group-hover:opacity-100 focus:opacity-100 disabled:opacity-50"
                      title="测试连接"
                    >
              ...
              <div className="grid grid-cols-1 gap-2 border-t border-gray-50 bg-gray-50/40 px-4 py-3 md:grid-cols-2">
                <label className="flex items-center justify-between gap-3 rounded-[14px] bg-white px-3 py-2 text-[12px] text-[#334155]">
                  <span className="flex items-center gap-2"><VolumeX className="h-3.5 w-3.5" /> 默认静音</span>
                  <input
                    type="checkbox"
                    checked={camera.mutedByDefault !== false}
                    onChange={(e) => handleUpdateCamera(camera.id, { mutedByDefault: e.target.checked })}
                    className="h-4 w-4 accent-[#334155]"
                  />
                </label>
                <label className="flex items-center justify-between gap-3 rounded-[14px] bg-white px-3 py-2 text-[12px] text-[#334155]">
                  <span className="flex items-center gap-2"><ShieldCheck className="h-3.5 w-3.5" /> 隐私模式</span>
                  <input
                    type="checkbox"
                    checked={camera.privacyMode === true}
                    onChange={(e) => handleUpdateCamera(camera.id, { privacyMode: e.target.checked })}
                    className="h-4 w-4 accent-[#334155]"
                  />
                </label>
              </div>
        ...
        <div className="bg-blue-50/50 rounded-[24px] p-5 border border-blue-50">
          ...
          <ul className="mt-3 space-y-2 text-[12px] text-blue-700/80 leading-relaxed">
            ...
            <li>• <b>隐私边界:</b> HAUI 不提供摄像头云转发服务，优先使用 HA 本地代理、go2rtc 或设备厂商通道。</li>
