...
HAUI 是面向 Home Assistant 的现代化控制面板，当前以 Add-on 交付为主。它提供设备控制、实时状态同步、AI 助手、Agent 控制台、摄像头监控、平板墙屏模式、离线 Pro 授权和备份恢复。

5.12.0 是面向正式交付的 Add-on 优先版本：推荐通过 Home Assistant 左侧栏使用，手机和平板可用 PWA 添加到主屏幕，暂不包含 Android/iOS 原生 App 包。

## 5.12.0 更新重点

- Add-on、根前端包、HA 自定义集成版本统一为 `5.12.0`。
- AI 聊天在 Ingress 中优先走 Add-on 后端代理，减少浏览器端 API Key 暴露。
- 授权激活支持 Add-on 后端机器码和验签，换浏览器或平板访问不会改变机器码。
- Pro 门控覆盖 AI、Agent、摄像头高级能力、墙屏/商业入口、备份恢复等能力；基础设备控制不受授权状态影响。
...
| 配置项 | 说明 |
| --- | --- |
| `HAUI_LICENSE_PUBLIC_KEY` | Pro 离线授权公钥。私钥只保存在开发者本机。 |
...
3. 更新 **HAUI - 智能家庭中枢** 到 `5.12.0`。
4. 启动 Add-on 后进入 HAUI，确认底部版本号显示 `HAUI v5.12.0`。
...
- 灯光、窗帘、空调、媒体、遥控和场景控制。
- 设备发现、设备映射、房间和人员管理。
- 基础 Home Assistant 控制不依赖 Pro 授权。
...
1. 检查 Add-on 配置是否填写 `HAUI_LICENSE_PUBLIC_KEY`。
2. 确认授权码机器码与授权页显示机器码一致。
3. 确认授权码来自当前私钥签名。

### AI 无法使用

1. 检查是否已激活包含 `ai` 或 `pro` 的授权。
