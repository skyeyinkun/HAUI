...
export interface DeviceTypeInfo {
    type: DeviceType;      // 内部类型 key
    label: string;         // 中文标签
    icon: string;          // MDI 图标名
    category: DeviceCategory;  // 分类
}
...
const DEVICE_TYPE_KEYWORDS: [string[], DeviceTypeInfo][] = [
    ...
    // 开关 - 归类为 other（开关类设备不属于 lighting/hvac/sensor/curtain/security）
    [['开关', '墙壁开关', '单开', '双开', '三开', '智能开关'],
    { type: 'switch', label: '开关', icon: 'toggle-switch', category: 'other' }],

    // 插座 - 归类为 other
    [['插座', '排插', '智能插座'],
    { type: 'outlet', label: '插座', icon: 'power-socket-eu', category: 'other' }],
    ...
    // 新风/加湿 - 使用 climate 类型（加湿器属于 HVAC 设备）
    [['新风', '加湿器', '除湿机', '净化器', '空气净化'],
    { type: 'climate', label: '新风净化', icon: 'air-purifier', category: 'hvac' }],

    // 门锁 - 使用 other 类型（门锁不在 DeviceType 定义中）
    [['门锁', '智能锁', '指纹锁', '密码锁'],
    { type: 'other', label: '门锁', icon: 'lock', category: 'security' }],
    ...
    // 电视 - 使用 other 类型
    [['电视', '投影', '投影仪'],
    { type: 'other', label: '电视', icon: 'television', category: 'other' }],

    // 音箱 - 使用 other 类型
    [['音箱', '音响', '智能音箱'],
    { type: 'other', label: '音箱', icon: 'speaker', category: 'other' }],

    // 扫地机 - 使用 other 类型
    [['扫地机', '扫地机器人', '吸尘器', '拖地机'],
    { type: 'other', label: '扫地机', icon: 'robot-vacuum', category: 'other' }],
    ...
    // 开关 - 归类为 other
        ...
        case 'input_boolean':
            if (deviceClass === 'outlet') {
                return { type: 'outlet', label: '插座', icon: 'power-socket-eu', category: 'other' };
            }
            return { type: 'switch', label: '开关', icon: 'toggle-switch', category: 'other' };
        case 'cover':
        ...
        case 'humidifier':
            return { type: 'climate', label: '加湿器', icon: 'air-humidifier', category: 'hvac' };
        case 'media_player':
            return { type: 'other', label: '媒体播放', icon: 'television', category: 'other' };
        case 'lock':
            return { type: 'other', label: '门锁', icon: 'lock', category: 'security' };
        case 'vacuum':
            return { type: 'other', label: '扫地机', icon: 'robot-vacuum', category: 'other' };
        case 'remote':
            return { type: 'remote', label: '遥控器', icon: 'remote', category: 'other' };
        case 'alarm_control_panel':
            return { type: 'other', label: '报警器', icon: 'shield-home', category: 'security' };
    // 人员追踪 - 归类为 other
        case 'person':
            return { type: 'other', label: '人员', icon: 'account', category: 'other' };
        case 'sensor':
...
/** 传感器子类型推断 */
function inferSensorType(name: string, deviceClass?: string, attributes?: Record<string, unknown>): DeviceTypeInfo {
...
export function extractEntityParams(domain: string, attributes: Record<string, unknown>): Record<string, unknown> {
