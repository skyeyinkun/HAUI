...
export function discoverDevicesFromStates(
    ...
    Object.values(entities).forEach(entity => {
        ...
        const category = typeInfo.category || legacyCategory;
        ...
        // Create Device - 使用辅助对象收集可选参数
        const optionalParams: Partial<Device> = {};
        if (entityParams.temperature !== undefined) optionalParams.temperature = entityParams.temperature as number;
        if (entityParams.current_temperature !== undefined) optionalParams.current_temperature = entityParams.current_temperature as number;
        if (entityParams.fan_mode) optionalParams.fan_mode = entityParams.fan_mode as string;
        if (entityParams.swing_mode) optionalParams.swing_mode = entityParams.swing_mode as string;
        if (entity.state !== 'off' && domain === 'climate') optionalParams.mode = entity.state as string;
        if (entityParams.brightness !== undefined) optionalParams.brightness = entityParams.brightness as number;
        if (entityParams.position !== undefined) optionalParams.position = entityParams.position as number;
        if (entityParams.supported_features !== undefined) optionalParams.supported_features = entityParams.supported_features as number;
