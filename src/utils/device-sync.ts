...
export const syncDevicesWithEntities = (
    ...
    const newDevices = currentDevices.map(device => {
        ...
        // 实体不存在（可能已被删除），标记为不可用
        if (!entity) {
            if (device.haAvailable !== false) {
                hasChanges = true;
                return { ...device, haAvailable: false };
            }
            return device;
        }
        ...
        if (device.type === 'light' || device.type === 'switch' || device.icon === 'lamp') {
            ...
            if (device.type === 'light' || device.icon === 'lamp') {
                ...
                // 修复：只有当HA明确返回亮度值时才更新，避免覆盖本地状态
                // 当灯开启但HA未返回亮度时，保持设备原有亮度值
                let nextBrightness = device.brightness;
                if (isOn) {
                    if (typeof rawBrightness === 'number') {
                        nextBrightness = rawBrightness;
                    }
                    // 如果灯开启但没有亮度值，默认设置为255（最大亮度）
                    else if (!device.brightness || device.brightness === 0) {
                        nextBrightness = 255;
                    }
                } else {
                    // 灯关闭时，亮度设为0
